 ...
 * USER MAPPING or FOREIGN TABLE that uses tdengine_fdw.
 *
 * Raise an ERROR if the option or its value is considered invalid.
 */
Datum
tdengine_fdw_validator(PG_FUNCTION_ARGS)
{
    ...
    /*
     * Check that only options supported by tdengine_fdw, and allowed for the
 ...
 */
bool
tdengine_is_valid_option(const char *option, Oid context)
{
    struct TDengineFdwOption *opt;
...
/*
 * Fetch the options for a tdengine_fdw foreign table.
 */
/*
 * 获取TDengine外部表的配置选项
 *
 ...
 *
 * 返回值:
 * 包含所有配置选项的tdengine_opt结构体指针
 *
 * 功能说明:
 * 1. 从外部表、服务器和用户映射中提取配置选项
 * 2. 合并所有选项到一个列表中
 * 3. 解析各个选项值并填充到tdengine_opt结构体中
 * 4. 设置默认值并验证必填选项
 */
tdengine_opt *
 ...
 *
 * 功能说明:
 *   1. 使用SplitIdentifierString函数解析逗号分隔的字符串
 *   2. 处理解析错误情况
 *   3. 返回解析后的标签列表
 *
 * 注意事项:
 *   - 输入字符串会被修改，所以需要先复制(pstrdup)
 */
static List *
tdengineExtractTagsList(char *in_string)
{
    List       *tags_list = NIL;
